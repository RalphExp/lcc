			...
			if (*token == '0' && (*rcp == 'x' || *rcp == 'X')) {
				...
				while (*++rcp) {
					if (map[*rcp]&DIGIT)
						...
						break;
					if (n&~(~0UL >> 4)) /* check the most significant 4 bits. */
						overflow = 1;
					else
						n = (n<<4) + d;
				}
				...
				for ( ; map[*rcp]&DIGIT; rcp++) {
					if (*rcp == '8' || *rcp == '9')
						err = 1;
					if (n&~(~0UL >> 3)) /* check the most significant 3 bits. */
						overflow = 1;
					else
						n = (n<<3) + (*rcp - '0');
				}
...
static void *scon(int q, void *put(int c, void *cl), void *cl) {
	...
	do {
		...
		while (*cp != q) {
			...
			if (map[*cp]&NEWLINE) {
				if (cp < limit) {
					// a line without a terminating quote is illegal
					break;
				}
				cp++;
				nextline();
    ...
    /* ch6: The above do-while loop gathers up adjacent string literals, which are
